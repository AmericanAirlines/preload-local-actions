...
#### Tracking Upstream
To track and sync the main (upstream) repo, run this command to create a remote: `git remote add upstream git@github.com:AmericanAirlines/preload-local-actions-action.git`. Whenever you need to sync your current branch with the `upstream main` branch, run `git pull upstream main`.
...
1. Use the `preload-local-actions-action:main` branch as your PR's base branch and select your branch as the `compare` branch
...
By contributing to `preload-local-actions-action`, you agree to the terms of American’s [Contributor License Agreement](https://github.com/AmericanAirlines/Contributor-License-Agreement/blob/main/CLA.md).
